...
router.beforeEach(async (to, from, next) => {
  ...
  if (localStorage.getItem('token')) {
    if (to.path === '/login') {
      next({ path: '/' })
      // 如果不是跳到登录页 并且有 角色和用户信息
    } else if (store.getters.roles.length > 0 && store.getters.userName) {
      if (store.getters.addRoutes.length === 0 && to.path !== '/403') {
        next('/403')
      } else {
        next()
      }
    ...
    //     'http://data-portal.bilibili.co/loginPage?caller=game-data-center&lc=111'
    // }
  }
})
